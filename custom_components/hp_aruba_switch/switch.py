...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for port in ports:
        ...
        if port not in exclude_ports:
            entities.append(
                ArubaSwitch(coordinator, port, False, config_entry.entry_id)
            )
...
class ArubaSwitch(ArubaSwitchEntity, SwitchEntity):
    ...
    def __init__(self, coordinator, port: str, is_poe: bool, entry_id: str):
        ...
        self._attr_unique_id = (
            f"{coordinator.host}_{port}_{'poe' if is_poe else 'port'}"
        )
    ...
    def is_on(self) -> bool:
        ...
        _LOGGER.debug(
            f"is_on property called for {self._attr_name}: {self._attr_is_on}"
        )
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if result is not None:
            ...
            _LOGGER.warning(
                f"Failed to turn on {self._attr_name} - switch may be offline"
            )
    ...
    async def async_turn_off(self, **kwargs):
        ...
        if result is not None:
            ...
            _LOGGER.warning(
                f"Failed to turn off {self._attr_name} - switch may be offline"
            )
    ...
    def _handle_coordinator_update(self):
        ...
        try:
            ...
            _LOGGER.debug(
                f"📊 Coordinator data keys: {list(self._coordinator.data.keys())}"
            )
            ...
            _LOGGER.debug(
                f"Port {self._port} {'PoE' if self._is_poe else ''} - live status: {status}, stats: {port_statistics}, link: {port_link_details}"
            )
            ...
            if self._is_poe:
                ...
                if power_enable and isinstance(poe_status_value, str):
                    poe_active = poe_status_value.lower() in [
                        "delivering",
                        "searching",
                        "on",
                        "enabled",
                ...
                _LOGGER.debug(
                    f"PoE port {self._port}: power_enable={power_enable}, poe_status={poe_status_value}, final_state={self._attr_is_on}"
                )
            else:
                ...
                self._attr_is_on = (
                    port_enabled  # Only check if port is administratively enabled
                )
                _LOGGER.debug(
                    f"Interface port {self._port}: port_enabled={port_enabled}, link_up={link_up}, final_state={self._attr_is_on}"
                )
            ...
            _LOGGER.debug(
                f"✅ Successfully updated {self._attr_name} - is_on: {self._attr_is_on}"
            )
