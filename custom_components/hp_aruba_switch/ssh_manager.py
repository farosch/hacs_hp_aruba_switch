...
class ArubaSSHManager:
            ...
            async with self._connection_lock:
                ...
                def _sync_execute():
                    ...
                    try:
                            ...
                            try:
                                ...
                                for i, cmd_line in enumerate(command_lines):
                                    if cmd_line.strip():  # Skip empty lines
                                        _LOGGER.debug(
                                            f"Sending command line {i+1}/{len(command_lines)}: {cmd_line.strip()}"
                                        )
                                ...
                                while time.time() - start_time < max_wait:
                                    if shell.recv_ready():
                                        ...
                                        if (
                                            "-- MORE --" in chunk
                                            or "next page: Space" in chunk
                                        ):
                                            _LOGGER.debug(
                                                "Detected pager prompt, sending space to continue"
                                            )
                                            shell.send(" ")  # Send space to continue
                                            time.sleep(0.5)
                                        elif (
                                            "(q to quit)" in chunk.lower()
                                            or "quit: control-c" in chunk.lower()
                                        ):
                                            _LOGGER.debug(
                                                "Detected quit prompt, sending 'q' to exit pager"
                                            )
                                ...
                                for line in lines:
                                    ...
                                    if (
                                        ...
                                        and not line.endswith(">")
                                        and "-- MORE --" not in line
                                        and "next page: Space" not in line
                                        and "quit: Control-C" not in line
                                        and "no page" not in line
                                        and command.replace("\n", " ").strip()
                                        not in line
                                    ):
                                ...
                                _LOGGER.debug(
                                    f"SSH command '{command}' output for {self.host}: {repr(output)}"
                                )
                                ...
                                if i == len(ssh_configs) - 1:  # Last attempt
                                    ...
                                    self._connection_backoff = min(
                                        self._connection_backoff * 1.5,
                                        self._max_backoff,
                                    )
                                    raise e
                                continue
                        ...
                        # Smaller backoff increase
                        self._connection_backoff = min(
                            self._connection_backoff * 1.5, self._max_backoff
                        )
                        raise
                    finally:
                        if ssh:
                ...
                try:
                    result = await asyncio.wait_for(
                        loop.run_in_executor(None, _sync_execute), timeout=timeout + 2
                    )
                    ...
                    if result is not None:
                        ...
                        if was_online:
                            _LOGGER.warning(
                                f"Switch {self.host} went offline (command returned no data)"
                            )
                    ...
                    _LOGGER.debug(
                        f"SSH command '{command}' failed for {self.host}: {e}"
                    )
                    was_online = self._is_available
                    self._is_available = False
                    if was_online:
                        _LOGGER.warning(
                            f"Switch {self.host} went offline (connection error: {e})"
                        )
                    return None

    async def get_all_switch_data(
        self,
    ) -> tuple[
        ...
        for cmd, parser in commands.items():
            ...
            try:
                ...
                if cmd == "show interface all":
                    ...
                    _LOGGER.debug(
                        f"✅ Parsed interface all: {len(ifaces)} interfaces, {len(stats)} stats, {len(links)} links"
                    )
                    ...
                    _LOGGER.debug(
                        f"✅ Parsed interface brief: {len(brief_info)} interfaces"
                    )
                ...
                _LOGGER.error(
                    f"❌ Parsing timed out for command '{cmd}' on {self.host}"
                )
            except Exception as err:
                _LOGGER.error(
                    f"❌ Parsing failed for command '{cmd}' on {self.host}: {err}"
                )
    ...
    def parse_show_interface_all(
        ...
        for raw_line in output.split("\n"):
            ...
            if (
                ...
                or (
                    "interface" in line_lower
                    and any(
                        x in line_lower for x in ["port", "ethernet", "gi", "fa", "te"]
                    )
                )
                ...
                try:
                    ...
                    if "port counters for port" in line_lower:
                        ...
                        match = re.search(
                            r"(?:interface|port|gi|fa|te)[\s]*(\d+)", line_lower
                        )
                        if match:
                    ...
                    if port_num:
                        ...
                        _LOGGER.debug(
                            f"Started parsing port {current_interface} from line: '{line}'"
                        )
                        in_totals_section = False
                        in_errors_section = False
                        in_others_section = False
            ...
            # Port enabled status
            if ("port enabled :" in normalized_line) or (
                "port enabled:" in normalized_line
            ):
                value_part = line.split(":", 1)[1].strip().lower()
                is_enabled = any(
                    pos in value_part
                    for pos in ["yes", "enabled", "up", "active", "true"]
                )
            ...
            # Link status
            if ("link status :" in normalized_line) or (
                "link status:" in normalized_line
            ):
                value_part = line.split(":", 1)[1].strip().lower()
                link_up = "up" in value_part
                interfaces[current_interface]["link_status"] = (
            ...
            # Port Name
            if normalized_line.startswith("name :") or normalized_line.startswith(
                "name:"
            ):
                parts = line.split(":", 1)
                if len(parts) == 2:
            ...
            if in_rates_section:
                ...
                if "unicast rx (pkts/sec)" in key or (
                    "unicast rx" in key and "pkts/sec" in key
                ):
                    numbers = extract_numbers(value_str)
                    if len(numbers) >= 2:
                ...
                if "b/mcast rx (pkts/sec)" in key or (
                    "b/mcast rx" in key and "pkts/sec" in key
                ):
                    numbers = extract_numbers(value_str)
                    if len(numbers) >= 2:
                ...
                if "utilization rx" in key:
                    ...
                    if "utilization tx" in value_str.lower():
                        tx_match = re.search(
                            r"utilization tx\s*:\s*([\d.]+)", value_str.lower()
                        )
                        if tx_match:
                            util_tx = float(tx_match.group(1))
                            statistics[current_interface][
                                "utilization_tx_percent"
                            ] = util_tx
                    continue
    ...
    def parse_show_interface_brief(self, output: str) -> Dict[str, Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if "port" in line.lower() and (
                "type" in line.lower() or "status" in line.lower()
            ):
                        ...
                        try:
                            ...
                            if len(right_fields) >= 4:
                                ...
                                brief_info[port_num] = {
                                    ...
                                    "intrusion_alert": (
                                        right_fields[1].lower() == "yes"
                                        if len(right_fields) > 1
                                        else False
                                    ),
                                    "enabled": (
                                        right_fields[2].lower() == "yes"
                                        if len(right_fields) > 2
                                        else False
                                    ),
                                    "flow_control": (
                                        right_fields[5].lower()
            ...
            for pattern in port_header_patterns:
                if pattern == r"^[0-9]+[/]*[0-9]*\s":
                    ...
                    try:
                        ...
                        if (
                            port_num
                            and port_num.replace("/", "").replace(".", "").isdigit()
                        ):
                ...
                for key, value in parsed_data.items():
                    ...
                    if "power enable" in key:
                        ...
                        if "searching" in value_lower:
                            ...
                            "enabled" in value_lower
                            or "on" in value_lower
                            or "active" in value_lower
                        ):
                            poe_status_str = "on"
                        elif (
                            "fault" in value_lower
                            or "error" in value_lower
                            or "overload" in value_lower
                        ):
                    ...
                    # Parse power and electrical values
                        ...
                        if match:
                            poe_ports[current_port]["pse_voltage"] = float(
                                match.group(1)
                            )
                        ...
                        match = re.search(r"(\d+)", value)
                        if match:
                            poe_ports[current_port]["pd_amperage_draw"] = int(
                                match.group(1)
                            )
                        ...
                        match = re.search(r"([\d.]+)", value)
                        if match:
                            poe_ports[current_port]["pd_power_draw"] = float(
                                match.group(1)
                            )
                        ...
                        match = re.search(r"([\d.]+)", value)
                        if match:
                            poe_ports[current_port]["pse_reserved_power"] = float(
                                match.group(1)
                            )
                    ...
                    # Parse LLDP power information
                        ...
                        match = re.search(r"([\d.]+)", value)
                        if match:
                            poe_ports[current_port]["lldp_pse_allocated"] = float(
                                match.group(1)
                            )
                        ...
                        match = re.search(r"([\d.]+)", value)
                        if match:
                            poe_ports[current_port]["lldp_pd_requested"] = float(
                                match.group(1)
                            )
                    ...
                    # Parse error/fault counters
                        ...
                        match = re.search(r"(\d+)", value)
                        if match:
                            poe_ports[current_port]["over_current_cnt"] = int(
                                match.group(1)
                            )
                        ...
                        match = re.search(r"(\d+)", value)
                        if match:
                            poe_ports[current_port]["power_denied_cnt"] = int(
                                match.group(1)
                            )
                        ...
                        match = re.search(r"(\d+)", value)
                        if match:
                            poe_ports[current_port]["mps_absent_cnt"] = int(
                                match.group(1)
                            )
    ...
    def parse_show_version(self, output: str) -> dict:
        ...
        _LOGGER.debug(
            f"🔍 VERSION PARSING: Processing {len(output)} characters of version output"
        )
        ...
        for line in output.split("\n"):
            ...
            if line.endswith("#"):
                ...
                if hostname:
                    ...
                    _LOGGER.debug(
                        f"🏷️ VERSION PARSING: Found hostname in prompt: {hostname} from line: {line}"
                    )
            ...
            if ":" in line:
                ...
                if len(parts) == 2:
                    ...
                    if any(
                        ...
                        _LOGGER.debug(
                            f"🔧 VERSION PARSING: Found boot ROM version: {value} from key: {key}"
                        )
                    elif any(x in key for x in ["model", "product", "type"]):
                        if (
                            "model" not in version_info
                        ):  # Don't override hostname-extracted model
            ...
            if "ya." in line_lower or "kb." in line_lower or "yc." in line_lower:
                ...
                if version_match:
                    ...
                    _LOGGER.debug(
                        f"📟 VERSION PARSING: Found version string: {version_str} from line: {line}"
                    )
                    ...
                    if len(version_str) > 8:  # YA.16.08.0002 is longer than YA.15.20
                        ...
                        _LOGGER.debug(
                            f"🎯 VERSION PARSING: Set as main firmware (length {len(version_str)}): {version_str}"
                        )
                    elif main_firmware_version is None:
                        main_firmware_version = version_str
                        _LOGGER.debug(
                            f"🔄 VERSION PARSING: Set as fallback firmware: {version_str}"
                        )
        ...
        if main_firmware_version:
            ...
            _LOGGER.debug(
                f"✅ VERSION PARSING: Using main firmware version: {main_firmware_version}"
            )
        elif "firmware_version" not in version_info and boot_version:
            version_info["firmware_version"] = boot_version
            _LOGGER.debug(
                f"⚠️ VERSION PARSING: Fallback to boot ROM version: {boot_version}"
            )
        ...
        if "model" not in version_info:
            ...
            _LOGGER.debug(
                f"⚠️ VERSION PARSING: No model found, using default: HP/Aruba Switch"
            )
    ...
    async def get_current_data(self) -> dict:
        ...
        try:
            ...
            interfaces, statistics, link_details, poe_ports, version_info = (
                await self.get_all_switch_data()
            )
    ...
    async def test_connectivity(self) -> bool:
        ...
        try:
            ...
            _LOGGER.info(
                f"Connectivity test for {self.host}: {'SUCCESS' if success else 'FAILED'}"
            )
            return success
        except Exception as e:
...
def get_ssh_manager(
    host: str, username: str, password: str, ssh_port: int = 22
) -> ArubaSSHManager:
