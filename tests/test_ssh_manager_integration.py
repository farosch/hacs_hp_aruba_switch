...
class TestRealSwitchIntegration:
    ...
    def ssh_manager(self):
        ...
        return ArubaSSHManager(
            REAL_SWITCH_IP,
            REAL_SWITCH_USERNAME,
            REAL_SWITCH_PASSWORD,
            22
        )
    ...
    async def test_parse_real_show_power_over_ethernet(self, ssh_manager):
        ...
        output = await ssh_manager.execute_command("show power-over-ethernet all", timeout=10)
        assert output is not None, "Command returned no output"
    ...
    async def test_get_all_switch_data_real(self, ssh_manager):
        ...
        interfaces, statistics, link_details, poe_ports, version_info = await ssh_manager.get_all_switch_data()
        
        # Should have data from at least some commands
        total_data = len(interfaces) + len(statistics) + len(link_details) + len(poe_ports) + len(version_info)
        ...
        if interfaces and statistics:
            ...
            assert interface_ports == stats_ports, "Interface and statistics ports should match"
